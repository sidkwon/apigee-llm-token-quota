...
apigeecli developers delete --email aidev-v2@cymbal.com --org "$PROJECT" --token "$TOKEN"

echo "Deleting API Products"
apigeecli products delete --name ai-product-bronze-v2 --org "$PROJECT" --token "$TOKEN"
...
apigeecli apis delete --name llm-token-limits-v2 --org "$PROJECT" --token "$TOKEN"
...
apigeecli datacollectors delete -n dc_candidates_token_count_v2 --org "$PROJECT" --token "$TOKEN"
apigeecli datacollectors delete -n dc_prompt_token_count_v2 --org "$PROJECT" --token "$TOKEN"
apigeecli datacollectors delete -n dc_total_token_count_v2 --org "$PROJECT" --token "$TOKEN"